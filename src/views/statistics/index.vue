<template>
  <div class="container">
    <el-table :data="tableData" :default-sort = "{prop: 'numberId', order: 'descending'}" style="width: 100%" max-height="250">
      <el-table-column
        fixed
        prop="numberId"
        sortable
...
export default {
  ...
  methods:{
    init(){
      ...
      this.wellId = this.userInfo.wellId
      if(this.wellId == 20) {
        getStatisticsList({pageNum:1,pageSize:20}).then(res => {
          this.tableData = res.data.list
          this.tableData.forEach((item,index) => {
            this.userNameFilters.push({value:item.userId,text:item.userName})
            this.wellNameFilters.push({value:item.wellId,text:item.wellName})
            this.timeFilters.push({value:item.timeLimit,text:item.timeLimit})
          })
          this.userNameFilters = this.unique(this.userNameFilters)
          this.wellNameFilters = this.unique(this.wellNameFilters)
          this.timeFilters = this.unique(this.timeFilters)
    ...
    prevClick(e){
      this.wellNameFilters = []
      this.userNameFilters = []
      getStatisticsList({pageNum:e,pageSize:20}).then(res => {
        this.tableData = res.data.list
        this.tableData.forEach((item,index) => {
          this.userNameFilters.push({value:item.userId,text:item.userName})
          this.wellNameFilters.push({value:item.wellId,text:item.wellName})
        ...
        this.userNameFilters = this.unique(this.userNameFilters)
        this.wellNameFilters = this.unique(this.wellNameFilters)
        this.timeLimit = this.unique(this.timeLimit)
        this.total = res.data.total
      })
    },
    nextClick(e){
